...
function ProductionBoard(props: any) {
  ...
  // let loggeduseremail = "nmcbrayer@goodtogreatschools.org.au";
  let currentpage = 1;
...
<ViewFields>
  ...
  const savePBData = () => {
    ...
    pbData.forEach((pb, Index: number) => {
      let strDWYNA: string = `${pb.DeveloperId}-${pb.Week}-${pb.Year}-${
        pb.NotApplicable != true && pb.NotApplicableManager != true
          ? false
          : true
      }`;
